...
InstallWebServer() {
  ...
  if [ $CFG_WEBSERVER == "apache" ]; then
	echo "Installing Apache and Modules... -->> PLEASE Wait <<-- "
	...
	apache2 -v
	echo -n "Installing PHP and Modules... -->> PLEASE Wait <<-- "
	...
	echo -e "${green}done!${NC}"
	php -v
	echo -n "Installing needed Programs for PHP and Apache... -->> PLEASE Wait <<-- "
